...
%% ─── Abstract ────────────────────────────────────────────────────────────────
\begin{abstract}
Industrial automation environments are characterised by a rich and heterogeneous
landscape of communication protocols—Modbus, OPC~UA, MQTT with Sparkplug~B,
BACnet, DNP3, EtherCAT, EtherNet/IP, PROFIBUS, PROFINET, and Siemens~S7comm—
that underpin the monitoring and control of physical processes in manufacturing,
energy, and infrastructure sectors. Contemporary large language model (LLM)
based AI assistants demonstrate strong capabilities in structured reasoning,
workflow orchestration, and natural language understanding, yet they lack any
native means to communicate with operational technology (OT) devices and
systems. This paper presents \textbf{IndustriConnect}, an open-source suite of
Model Context Protocol (MCP) servers that bridges this gap by exposing all nine
major industrial protocols as structured, self-describing AI tools. Each
protocol module follows a consistent three-layer architecture in which an MCP
client (such as a Claude~Desktop session or a web management console) invokes
tool calls that are translated by a Python MCP server into native protocol
operations against real or simulated field devices. Key design contributions
include: (i)~a unified response envelope \texttt{\{success, data, error, meta\}}
that enables cross-protocol observability and consistent error handling;
(ii)~a mock-first development methodology in which every protocol module ships
with a high-fidelity device simulator for safe, hardware-free iteration;
(iii)~industrial-grade reliability patterns including automatic chunking,
exponential-backoff retries, and typed data access; and (iv)~a companion
web-based management console (\textit{mcp-manager-ui}) supporting
multi-server configuration and LLM-driven interaction. The suite is built on
the FastMCP framework for Python and is released as open source. Together
these components enable AI assistants to read process values, write setpoints,
browse device hierarchies, and orchestrate diagnostic workflows across
heterogeneous OT environments, without requiring dedicated integration
middleware or manual protocol programming.
\end{abstract}

\textbf{Keywords:} industrial automation; Model Context Protocol; large language
models; OPC~UA; Modbus; IIoT; operational technology; AI-assisted control;
MQTT; PROFINET.
...
\url{https://github.com/[your-repo-path-here]}. Contributions in the form of
